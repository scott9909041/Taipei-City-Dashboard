...
function handleRemoveMap(index) {
	adminStore.removeMap({ index });
}
</script>
			...
			<div class="admincomponentsettings-content">
				<div class="admincomponentsettings-settings">
					...
					<div v-else-if="currentSettings === 'map'">
						<div class="admincomponentsettings-mapconfig">
							<button @click="handleAddMap">新增地圖</button>
						</div>
						<div
							v-for="(
								map_config, index
						...
						>
							<hr v-if="index > 0" />
							<div class="admincomponentsettings-map-header">
								<label>地圖{{ index + 1 }} ID / Index</label>
								<button @click="handleRemoveMap(index)">
									刪除地圖{{ index + 1 }}
								</button>
							</div>
							<div class="two-block">
